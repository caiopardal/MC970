    ...
    plasma_sequence_t *sequence, plasma_request_t *request)
{
    ...
    if (sequence->status == PlasmaSuccess) {
        int side_ = side, uplo_ = uplo;
        int transa_ = transa, diag_ = diag;
        int size_A = lda*ak, size_B = ldb*n;

        #pragma omp target nowait                                   \
                depend(in:A[0:size_A])                              \
                depend(inout:B[0:size_B])                           \
                firstprivate(side_, uplo_, transa_, diag_)          \
                firstprivate(m, n, alpha, lda, ldb)                 \
                map(to:A[0:size_A])                                 \
                map(tofrom:B[0:size_B])
        {
            int block_size = 64;
