    ...
    plasma_sequence_t *sequence, plasma_request_t *request)
{
    ...
    if (sequence->status == PlasmaSuccess)
    {
        ...
        #pragma omp target nowait                               \
            depend(in:A[0:size_A])                              \
            depend(in:B[0:size_B])                              \
            depend(inout:C[0:size_C])                           \
        ...
        {
            int block_size = 64, size_matrix = lda;
            int m_new = m/block_size;
            int n_new = n/block_size;
            int k_new = k/block_size;
            int s_block_size = block_size*block_size;
            ...
            #pragma omp single
            {
                for (int m_ = 0; m_ < m_new; m_++){
                    ...
                    for (int k_ = 0; k_ < k_new; k_++) { 	
                        int lda1_ = m_ * block_size;
                        int lda2_ = lda1_ + k_ * block_size;
                        int ldb1_ = k_ * block_size;
                        int ldb2_ = ldb1_ + n_ * block_size;
                        int ldc1_ = m_ * block_size;
                        int ldc2_ = ldc1_ + n_ * block_size;
                        #pragma omp task                            \
                            depend(in:A[lda1_:lda2_])               \
                            depend(in:B[ldb1_:ldb2_])               \
                            depend(inout:C[ldc1_:ldc2_]) 
                        {
                            float A_new [s_block_size],B_new[s_block_size],C_new[s_block_size];
                            int i_a = m_, j_a=k_;
                            int i_b = k_, j_b=n_;
                            int i_c = m_, j_c=n_;
                            int insert = 0;	
